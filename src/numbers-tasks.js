...
function getAverage(value1, value2) {
  const adaptedArr = [value1, value2].map((arg) => arg / 2);
  const avg = adaptedArr.reduce((acc, curr) => {
    const sum = acc + curr;
    return sum;
  }, 0);
  return avg;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalarProizvedenie = x1 * x2 + y1 * y2;
  const length1 = Math.sqrt(x1 * x1 + y1 * y1);
  const length2 = Math.sqrt(x2 * x2 + y2 * y2);
  if (length1 === 0 || length2 === 0) {
    return 0;
  }
  const cosTheta = scalarProizvedenie / (length1 * length2);
...
function getSumOfDigits(num) {
  ...
  let decreasingNum = num;
  let sum = 0;
  while (decreasingNum > 0) {
    const rest = decreasingNum % 10;
    sum += rest;
    decreasingNum /= 10;
    decreasingNum = Math.floor(decreasingNum);
  }
  return sum;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return [x1, x2, x3].reduce((acc, curr) => acc + curr, 0).toFixed(1);
}
